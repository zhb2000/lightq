...
class TypeRouterMixin(Generic[Handler]):
    def __init__(self):
        self.type_to_handlers: dict[type, list[Handler]] = {}

    def build(self, handlers: Iterable[Handler]):
        self.clear()
        for handler in handlers:
            for cls in handler.types:
    ...
    async def _route_by_exact_type(self, cls: type, context) -> Handler | None:
        for handler in self.type_to_handlers.get(cls, []):
...
class MessageTypeRouter(TypeRouterMixin[MessageHandler], MessageRouter):
    type_to_handlers: dict[type[Message], list[MessageHandler]]
...
class ExceptionTypeRouter(TypeRouterMixin[ExceptionHandler], ExceptionRouter):
    type_to_handlers: dict[type[Exception], list[ExceptionHandler]]
    ...
    def __init__(
        ...
        TypeRouterMixin.__init__(self)
        ExceptionRouter.__init__(self, before, after)
